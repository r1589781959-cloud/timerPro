...
class PerfectTimerApp:
    def __init__(self, root):
        ...
        self.tabs = ["全部订单", "先玩后付", "无限模式", "定额/倒计时", "限时段", "团购", "超时监控"]
    ...
    def calculate_single_bill(self, gid, end_time):
        ...
        if d.get('is_suspended', False):
            ...
            if d['mode'] == 'group_buy':
                ...
                lm = d['limit_min']
                added_time = d.get('added_time_min', 0)
                # 结账界面的“定额时长”按初始定额显示（不叠加中途加时/加团购）
                fixed_time_str = fmt_min(lm)
        ...
        # 不限时类型团购（unlimited, single_board）不应该计算加时费用
        added_time_cost = 0.0
        if d['mode'] not in ['fixed', 'group_buy', 'unlimited', 'single_board']:
    ...
    def manage_added_gb_verification(self, gid):
        ...
        def verify_selected():
            ...
            added_lines = []
            for gb in data.get('added_gb', []):
                status_text = "已核销" if gb.get('verify_status') else "未核销"
                add_time = gb.get('add_time', '')
                name = gb.get('name', '')
                price = float(gb.get('price', 0.0) or 0.0)
                added_lines.append(f"【加团购】{add_time} {name} ({status_text}) ¥{price:.2f}".strip())
        ...
        def confirm():
            try:
                ...
                if mode == "direct":
                    ...
                    new_remark = f"{current_remark}\n【加时】{add_time_str} +{add_min}分钟 ¥{add_price:.2f}" if current_remark else f"【加时】{add_time_str} +{add_min}分钟 ¥{add_price:.2f}"
                    ...
                    current_remark = data.get('remark', '').strip()
                    new_remark = f"{current_remark}\n【加团购】{add_time_str} {selected_gb['name']} ({verify_status_text}) ¥{add_price:.2f}" if current_remark else f"【加团购】{add_time_str} {selected_gb['name']} ({verify_status_text}) ¥{add_price:.2f}"
                    data['remark'] = new_remark.strip()
    ...
    def show_single_checkout_ui(self, b):
        ...
        lbl_fixed_detail = None

        checkout_win = tk.Toplevel(self.root)
        checkout_win.title("结账/挂账详情")
        checkout_win.geometry("480x950")
        checkout_win.transient(self.root)
        checkout_win.grab_set()
        tk.Label(checkout_win, text=f"顾客: {b['data']['phone']} [👥 {b['data'].get('guest_count', 1)}人]",
                 font=("微软雅黑", 18, "bold")).pack(pady=15)

        f_detail = tk.Frame(checkout_win, bg="#f0f0f0", padx=10, pady=10)
        f_detail.pack(fill="x", padx=20)

        def row_in(parent, k, v, color="black"):
        ...
        def row(k, v, color="black"):
            row_in(f_detail, k, v, color)

        row("计费模式:", b['mode_info'])
        if b['fixed_str'] != "--": row("初始定额时长:", b['fixed_str'])
        
        # 计算并显示总定额时长
        total_fixed_min = 0
        if b['data']['mode'] == 'group_buy' and b['data'].get('gb_config'):
            # 团购模式：基础时长 + 已添加时间
            # 注意：加团购时长已经计入 added_time_min，不能再额外叠加一次，否则会重复计算
            base_min = b['data']['gb_config'].get('limit_min', 0)
            added_time_min = b['data'].get('added_time_min', 0)
            total_fixed_min = base_min + added_time_min
        elif b['data']['mode'] == 'fixed':
            # 普通定额模式同上：added_time_min 已包含加团购时长
            base_min = b['data'].get('limit_min', 0)
            added_time_min = b['data'].get('added_time_min', 0)
            total_fixed_min = base_min + added_time_min
        
        # 只有在有定额时长的情况下才显示总定额时长（不限时类型不显示）
        if total_fixed_min > 0 and b['data']['mode'] not in ['unlimited', 'single_board']:
            row("总定额时长:", fmt_min(total_fixed_min))
        ...
        # 添加团购券显示区域（只显示一次，避免重复）
        f_added_gb = tk.Frame(f_detail, bg="#f0f0f0")
        f_added_gb.pack(fill="x", pady=2)

        def render_added_gb_section():
            for child in f_added_gb.winfo_children():
                child.destroy()
            if 'added_gb' not in b['data'] or not b['data']['added_gb']:
                return
            row_in(f_added_gb, "添加团购券:", f"{len(b['data']['added_gb'])} 张")
            for i, gb in enumerate(b['data']['added_gb']):
                status = "已核销" if gb.get('verify_status') else "未核销"
                color = "#27ae60" if gb.get('verify_status') else "#e74c3c"
                name = gb.get('name', '')
                price = float(gb.get('price', 0.0) or 0.0)
                row_in(f_added_gb, f"  {i+1}. {name}:", f"¥{price:.2f} ({status})", color)
        ...
        if b['data']['mode'] == 'group_buy':
            ...
            is_verified_init = bool(b['data'].get('gb_verified', False))
            lbl_verify_status = tk.Label(
                f_verify_status,
                text=("✓ 已核销" if is_verified_init else "✗ 未核销"),
                bg="#f0f0f0",
                fg=("#27ae60" if is_verified_init else "#e74c3c"),
                                         font=("微软雅黑", 10, "bold"))
        ...
        # 显示常规定额卡片的信息
            ...
            c = self.config
            lm = b['data']['limit_min']
            added_time = b['data'].get('added_time_min', 0)
            base_cost = round((lm / 60.0) * c['price_overtime'], 2)
            added_time_cost_display = round((added_time / 60.0) * c['price_overtime'], 2)

            row("基础定额费:", f"{base_cost:.2f} 元", "#2980b9")
            if added_time > 0:
                row("加时费用:", f"{added_time_cost_display:.2f} 元", "#3498db")

            over = max(0, b['minutes'] - (lm + added_time))
            overtime_cost = 0
            if over > c['buffer_min']:
                overtime_cost = self.get_overtime_cost(over, c['price_overtime'], c)
                row("超时费用:", f"{overtime_cost:.2f} 元", "#e74c3c")

            if 'added_gb' in b['data'] and b['data']['added_gb']:
                gb_minutes = sum(gb['minutes'] for gb in b['data']['added_gb'])
                gb_time_cost = round((gb_minutes / 60.0) * c['price_overtime'], 2)
                row("团购时长抵扣:", f"-{gb_time_cost:.2f} 元", "#27ae60")
        
        # 显示其他模式的加时费用（不包括不限时类型）
        elif b['data']['mode'] not in ['unlimited', 'single_board']:
            added_time_cost = b['data'].get('added_time_cost', 0.0)
            if added_time_cost > 0:
                row("加时费用:", f"¥{added_time_cost:.2f} 元", "#3498db")

        # 渲染“添加团购券”区域（只渲染一次）
        render_added_gb_section()
        ...
        if 'added_gb' in b['data'] and b['data']['added_gb']:
            def manage_added_gb():
                ...
                dlg.grab_set()
                dlg.transient(checkout_win)
                dlg.lift()
                
                # 创建列表框
                listbox = tk.Listbox(dlg, width=40, height=10)
                listbox.pack(pady=10)
                ...
                def verify_selected():
                    selected = listbox.curselection()
                    if not selected:
                        messagebox.showinfo("提示", "请选择要核销的团购券", parent=dlg)
                        return
                    
                    index = selected[0]
                    b['data']['added_gb'][index]['verify_status'] = True

                    # 更新备注：仅更新“加团购”行，保留其它备注
                    old_remark = b['data'].get('remark', '')
                    preserved_lines = [ln for ln in old_remark.splitlines() if not ln.strip().startswith("【加团购】")]
                    preserved = "\n".join(preserved_lines).strip()
                    added_lines = []
                    for gb in b['data'].get('added_gb', []):
                        status_text = "已核销" if gb.get('verify_status') else "未核销"
                        add_time = gb.get('add_time', '')
                        name = gb.get('name', '')
                        price = float(gb.get('price', 0.0) or 0.0)
                    ...
                    # 更新备注显示
                    entry_remark.delete(1.0, tk.END)
                    entry_remark.insert(1.0, b['data'].get('remark', ''))
                    
                    # 重新计算费用
                    new_bill = self.calculate_single_bill(b['gid'], datetime.now())
                    var_total.set(round(new_bill['actual_total'], 2))
                    # 只有团购模式才需要更新核销状态显示
                    if b['data']['mode'] == 'group_buy' and 'update_total_price' in locals():
                        update_total_price()
                    if b['data']['mode'] == 'fixed' and lbl_fixed_detail is not None:
                        try:
                            update_fixed_detail()
                        except:
                            pass
                    ...
                    self.save_to_disk()
                    dlg.destroy()
                    messagebox.showinfo("成功", "团购券已核销", parent=checkout_win)
                    checkout_win.lift()
                    checkout_win.focus_force()
                
                # 取消核销按钮
                def unverify_selected():
                    selected = listbox.curselection()
                    if not selected:
                        messagebox.showinfo("提示", "请选择要取消核销的团购券", parent=dlg)
                        return
                    
                    index = selected[0]
                    b['data']['added_gb'][index]['verify_status'] = False
                    ...
                    self.save_to_disk()
                    dlg.destroy()
                    messagebox.showinfo("成功", "团购券已取消核销", parent=checkout_win)
                    checkout_win.lift()
                    checkout_win.focus_force()
                ...
                tk.Button(button_frame, text="核销选中的团购券", bg="#27ae60", fg="white", command=verify_selected).pack(side="left", padx=10)
                tk.Button(button_frame, text="取消核销选中的团购券", bg="#e74c3c", fg="white", command=unverify_selected).pack(side="left", padx=10)
                tk.Button(dlg, text="关闭", bg="#95a5a6", fg="white", command=dlg.destroy).pack(pady=10)
            
            tk.Button(checkout_win, text="管理添加的团购券", bg="#3498db", fg="white", font=("微软雅黑", 10),
                     command=manage_added_gb).pack(pady=10)
        ...
        tk.Label(checkout_win, text="订单备注:", font=("微软雅黑", 10)).pack(pady=(10, 0))
        entry_remark = tk.Text(checkout_win, font=("微软雅黑", 10), height=3, width=30, bd=1, relief="solid")
        ...
        # 普通定额模式：显示费用明细
        if b['data']['mode'] == 'fixed':
            lbl_fixed_detail = tk.Label(checkout_win, text="", font=("微软雅黑", 10), wraplength=420)

            def update_fixed_detail(*args):
                try:
                    c = self.config
                    lm = b['data']['limit_min']
                    added_time = b['data'].get('added_time_min', 0)
                    ...
                    new_total = var_total.get()
                    parts = [f"定额费 ¥{base_cost:.2f}"]
                    if added_time > 0:
                        parts.append(f"加时费 ¥{added_time_cost_display:.2f}")
                    if overtime_cost > 0:
                        parts.append(f"超时费 ¥{overtime_cost:.2f}")
                    if added_gb_cost > 0:
                        parts.append(f"未核销团购 ¥{added_gb_cost:.2f}")
                    if gb_time_cost > 0:
                        parts.append(f"团购抵扣 -¥{gb_time_cost:.2f}")

                    detail_str = " + ".join(parts) + f" = ¥{new_total:.2f}"
                    lbl_fixed_detail.config(text=f"📌 费用明细：{detail_str}", fg="#2c3e50")
                except Exception as e:
                    print(f"更新定额明细时出错: {e}")

            lbl_fixed_detail.pack(pady=(5, 5))
            update_fixed_detail()
        
        # 团购核销状态修改（必须在金额定义之后）
        verify_var = None
        if b['data']['mode'] == 'group_buy':
            tk.Label(checkout_win, text="核销管理:", font=("微软雅黑", 10, "bold")).pack(pady=(15, 5))
            f_verify_frame = tk.Frame(checkout_win)
            ...
            lbl_actual_total = tk.Label(checkout_win, text="", font=("微软雅黑", 10))
            ...
            def update_total_price(*args):
                ...
                try:
                    ...
                    if lbl_actual_total is not None:
                        if is_verified:
                            lbl_actual_total.config(
                                text=f"📌 费用明细：超时费 ¥{b['gb_extra_cost']:.2f} + 未核销添加团购 ¥{added_gb_cost:.2f} + 加时费用 ¥{added_time_cost:.2f} = ¥{new_total:.2f}（券价已收）",
                                fg="#27ae60"
                            )
                        else:
                            lbl_actual_total.config(
                                text=f"📌 费用明细：券价 ¥{b['gb_voucher_price']:.2f} + 超时费 ¥{b['gb_extra_cost']:.2f} + 未核销添加团购 ¥{added_gb_cost:.2f} + 加时费用 ¥{added_time_cost:.2f} = ¥{new_total:.2f}",
                                fg="#e74c3c"
                            )
        ...
        def suspend_action():
            try:
                ...
                messagebox.showerror("错误", "金额无效", parent=checkout_win); return
        ...
        def confirm():
            ...
            self.refresh_ui_list()
            checkout_win.destroy()
            messagebox.showinfo("完成", f"结账成功\n入账金额: {final_total}元", parent=self.root)

        f_btns = tk.Frame(checkout_win)
        f_btns.pack(pady=20)
    ...
    def create_guest_ui(self, gid, phone, mode, start_time, limit_min, remark="", is_paused=False, group_id=None,
        ...
        tk.Button(bg_frame, text="结账/挂账", bg="#e74c3c", fg="white", font=("微软雅黑", 12, "bold"),
                  command=lambda: self.checkout(gid), width=8).pack(side="right")
    ...
    def on_tab_change(self, event):
        ...
        if cur_tab == "超时监控": self.notebook.set_badge(6, False)
        if cur_tab != "全部订单":
            self.active_group_filter = None
        self.refresh_ui_list()
    ...
    def open_history_window(self):
        ...
        filter_frame.pack(fill="x", padx=10, pady=5, side="top")

        tk.Label(filter_frame, text="关键词:").pack(side="left")
        search_var = tk.StringVar()
        tk.Entry(filter_frame, textvariable=search_var, width=12).pack(side="left", padx=5)

        tk.Label(filter_frame, text="起始日期:").pack(side="left", padx=(10, 0))
        date_start_var = tk.StringVar()
        tk.Entry(filter_frame, textvariable=date_start_var, width=11).pack(side="left", padx=2)
        tk.Label(filter_frame, text="结束日期:").pack(side="left", padx=(5, 0))
        date_end_var = tk.StringVar()
        tk.Entry(filter_frame, textvariable=date_end_var, width=11).pack(side="left", padx=2)
        tk.Button(filter_frame, text="按日期筛选", bg="#3498db", fg="white", font=("微软雅黑", 9),
                  command=lambda: filter_data()).pack(side="left", padx=5)
        tk.Button(filter_frame, text="清除日期", bg="#95a5a6", fg="white", font=("微软雅黑", 9),
                  command=lambda: [date_start_var.set(""), date_end_var.set(""), filter_data()]).pack(side="left", padx=2)

        tk.Button(filter_frame, text="导出Excel", bg="#27ae60", fg="white", font=("微软雅黑", 9),
                  command=lambda: export_data()).pack(side="right", padx=5)
        tk.Button(filter_frame, text="批量删除选中", bg="#e74c3c", fg="white", font=("微软雅黑", 9),
                  command=lambda: delete_selected()).pack(side="right", padx=5)

        # ===== 统计栏 =====
        stats_frame = tk.Frame(win, bg="#ecf0f1", pady=10)
        stats_frame.pack(fill="x", padx=10, side="top")
        lbl_count = tk.Label(stats_frame, text="资流总数: 0 人", font=("微软雅黑", 12, "bold"), fg="#2980b9",
                             bg="#ecf0f1")
        lbl_count.pack(side="left", padx=20)
        lbl_gb_total = tk.Label(stats_frame, text="团购价值: ¥0.00", font=("微软雅黑", 12, "bold"), fg="#8e44ad",
                                bg="#ecf0f1")
        lbl_gb_total.pack(side="left", padx=20)
        lbl_settle = tk.Label(stats_frame, text="结算金额: ¥0.00", font=("微软雅黑", 12, "bold"), fg="#e67e22",
                              bg="#ecf0f1")
        lbl_settle.pack(side="left", padx=20)
        lbl_income = tk.Label(stats_frame, text="核算总金额: ¥0.00", font=("微软雅黑", 12, "bold"), fg="#27ae60",
                              bg="#ecf0f1")
        lbl_income.pack(side="right", padx=20)

        # ===== 表格 =====
        cols = (
            "start", "end", "id", "phone", "mode", "tot_dur", "play_dur", "cost", "gb_type", "gb_val", "settle",
            "remark", "fixed", "pause")
        tree = ttk.Treeview(win, columns=cols, show='headings', selectmode='extended')
        headers = ["开始", "结束", "序号", "标识", "模式", "总时长", "实玩时长", "总金额", "团购类型", "团购价值", "结算金额",
                   "备注", "定额", "暂停"]
        ...
        tree.pack(fill="both", expand=True, padx=10, pady=10, side="top")
        ...
        def filter_data(*args):
            ...
            for row in reversed(all_rows):
                # 行格式补齐
                if len(row) == 12:
                    new_row = [row[0], row[1], row[2], row[3], row[4], row[6], row[7], row[8], "", "0", row[8],
                               row[9], row[10], row[11]]
                    row = new_row
                elif len(row) < 14:
                    row += ["--"] * (14 - len(row))
                ...
                # 日期过滤（以开单时间为准，跨天以开单日计）
                if ds or de:
                    try:
                        row_date = row[0][:10] if len(row[0]) >= 10 else row[0]
                        if ds and row_date < ds:
                            continue
                        if de and row_date > de:
                            continue
                    except:
                        pass
        ...
        def delete_selected():
            ...
            if not selected:
                return
            if messagebox.askyesno("删除", f"删除选中的 {len(selected)} 条记录？", parent=win):
                ...
                if os.path.exists(HISTORY_FILE):
                    with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
                        ...
                        for row in r:
                            match = False
                            for d in to_del:
                                if len(row) > 3 and row[0] == d[0] and row[2] == d[2] and row[3] == d[3]:
                                    match = True
                                    break
                            if not match:
                                new_rows.append(row)
        ...
        def export_data():
            ...
            if not HAS_OPENPYXL:
                messagebox.showerror("缺少依赖",
                                     "导出Excel功能需要 openpyxl 库。\n请在命令行运行: pip install openpyxl\n安装后重启程序即可使用。",
                                     parent=win)
                return

            # 选择导出范围
            choice_win = tk.Toplevel(win)
            choice_win.title("选择导出范围")
            choice_win.geometry("320x180")
            choice_win.grab_set()
            choice_win.transient(win)
            # 居中
            choice_win.update_idletasks()
            x = win.winfo_x() + (win.winfo_width() - choice_win.winfo_width()) // 2
            y = win.winfo_y() + (win.winfo_height() - choice_win.winfo_height()) // 2
            choice_win.geometry(f"+{x}+{y}")

            tk.Label(choice_win, text="选择导出范围:", font=("微软雅黑", 12, "bold")).pack(pady=(15, 5))
            export_scope = tk.StringVar(value="current")
            tk.Radiobutton(choice_win, text="导出当前显示的记录", variable=export_scope, value="current",
                           font=("微软雅黑", 11)).pack(anchor="w", padx=40)
            tk.Radiobutton(choice_win, text="导出全部记录", variable=export_scope, value="all",
                           font=("微软雅黑", 11)).pack(anchor="w", padx=40)
            ...
            def do_export():
                ...
                filepath = filedialog.asksaveasfilename(
                    ...
                    initialfile=f"账单导出_{datetime.now().strftime('%Y%m%d_%H%M')}"
                )
                if not filepath:
                    return
                ...
                try:
                    ...
                    headers = ["开始", "结束", "序号", "标识", "模式", "总时长", "实玩时长", "总金额", "团购类型",
                               "团购价值", "结算金额", "备注", "定额", "暂停"]
                    ws.append(headers)
                    ...
                    wb.save(filepath)
                    messagebox.showinfo("导出成功", f"账单已导出到:\n{filepath}", parent=win)
                except Exception as e:
                    messagebox.showerror("导出失败", f"导出时发生错误:\n{e}", parent=win)

            tk.Button(choice_win, text="确认导出", bg="#27ae60", fg="white", font=("微软雅黑", 11, "bold"),
