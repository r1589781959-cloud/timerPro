...
DROP TABLE IF EXISTS shops; DROP TABLE IF EXISTS users; DROP TABLE IF EXISTS system_configs;
DROP TABLE IF EXISTS group_buys; DROP TABLE IF EXISTS orders; DROP TABLE IF EXISTS order_pause_logs;
DROP TABLE IF EXISTS order_add_times; DROP TABLE IF EXISTS order_group_buys;
...
if os.path.exists(json_path):
    ...
    for oid, s in old_data.get('g', {}).items():
        ...
        try:
            ...
            ''', (
                s.get('phone'), s.get('mode'), s.get('start_time'), s.get('limit_min', 0), 
                int(s.get('is_paused', 0)), int(s.get('is_suspended', 0)), 
                s.get('total_pause_sec', 0), s.get('guest_count', 1), 
                s.get('remark', ''), now, now
            ))
        except: pass
